 ...
 */
function buildCycleTimeline(arpConfig) {
    ...
    return arpConfig.cycle_times.map((time, i) => {
        ...
        if (cycleNum === 1)
            label = "Wake (ARP)";
        if (time === onset5)
            label = "Sleep onset — 5 cycles";
        if (time === onset4)
            label = "Sleep onset — 4 cycles";
        if (time === onset3)
            label = "Sleep onset — 3 cycles (floor)";
        if (time === crpOpen)
            label = "CRP window opens";
        if (time === crpClose)
            label = "CRP window closes / Phase 3";
        return {
            cycle: cycleNum,
            time,
            phase,
 ...
 */
function buildNotificationSchedule(arpConfig) {
    const schedule = [];
...
function isBetween(t, open, close) {
...
function addNinetyMinutes(hhmm) {
...
export function buildDayPlanPayload(arpConfig, date) {
    return {
        date,
...
// ─── Service function ──────────────────────────────────────────────────────────
export async function getDayPlanPayload(client, userId, date) {
...
//# sourceMappingURL=day-plan.js.map