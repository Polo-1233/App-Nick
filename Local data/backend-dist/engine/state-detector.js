 ...
 */
import { consecutiveNightsBelow, countHighLatencyNights, countCaffeineAfterCutoff, meanMRMCount, countCRPTaken, } from "./weekly-accounting.js";
...
function makeState(id, priority, activeDays = 0, confidence = "HIGH") {
    return {
        state_id: id,
        priority,
...
function activeDaysFor(_stateId, _ctx) {
...
function detectUS04(ctx) {
    const { sleep_logs, profile } = ctx;
    if (!profile.arp_committed)
        return null; // US-12 already handles no-ARP case
    const wakeTimes = sleep_logs.slice(0, 7).map(l => l.wake_time);
    const validWakeTimes = wakeTimes.filter((t) => t != null);
    if (validWakeTimes.length < 3)
...
function detectUS02(ctx, us03Active) {
    if (us03Active)
        return null; // US-03 supersedes
...
function detectUS08(ctx) {
    ...
    for (const log of sleep_logs.slice(0, 5)) {
        if (log.actual_sleep_onset == null)
            continue;
        ...
        if (onsetMin < targetOnsetMin - 360)
            onsetMin += 1440;
        if (onsetMin > lateOnsetThreshold)
            chronicLateOnset++;
    }
...
function detectUS11(ctx) {
    ...
    let score = 0;
    if (environment.bedroom_temperature === "hot" || environment.bedroom_temperature === "variable")
        score++;
    if (environment.evening_light_environment === "bright_blue")
        score++;
    if (environment.tv_in_bedroom)
        score++;
    if (environment.work_items_in_bedroom)
        score++;
    if (environment.blackout_provision && !environment.dws_device)
        score++;
    if (score >= 2) {
        return makeState("US-11", 3, activeDaysFor("US-11", ctx));
    }
...
function detectUS06(ctx, us03Active) {
    if (us03Active)
        return null; // US-03 takes priority
    const disruptionTypes = ["travel", "social_disruption", "shift_change"];
    const activeEvents = ctx.events.filter(e => e.active &&
        disruptionTypes.includes(e.event_type));
...
function detectUS16(ctx) {
    const illnessEvents = ctx.events.filter(e => e.active && (e.event_type === "illness" || e.event_type === "injury"));
...
// ─── Pass 6: Maintenance ──────────────────────────────────────────────────────
/** US-01 — Aligned */
function detectUS01(ctx, activeStates) {
    ...
    if (!weekly_balance)
        return null;
    if (weekly_balance.weekly_cycle_total >= 33 &&
 ...
 */
export function detectStates(ctx) {
    const states = [];
    // ── Pass 1: Gate States ──────────────────────────────────────────────────
...
export function isActive(states, id) {
    return states.some(s => s.state_id === id);
}
/** Find a state by ID */
export function findState(states, id) {
