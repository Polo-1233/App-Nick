 ...
 */
export function computeToneOverride(states) {
 ...
 */
export function applySuppressionRules(candidates, states) {
    const suppressed = [];
...
function shouldSuppress(rec, states, us07, us09, us12, us16) {
    // ── US-07: Sleep Anxiety Loop → Hard suppressions ────────────────────────
    if (us07) {
        // HARD: never show sleep restriction when anxiety is active
        if (rec.rec_type === "REC-19")
            return "US-07_active";
        // Suppress tracker recommendations
        if (rec.rec_type === "REC-18")
            return "US-07_suppress_tracker_rec";
        ...
        if (rec.rec_type === "REC-08")
            return "US-07_active";
        // Suppress weekly balance review (outcome metric)
        if (rec.rec_type === "REC-14")
            return "US-07_suppress_outcome_metrics";
    }
    // ── US-09: Ortho-Insomnia → suppress environment audit ────────────────────
    // Do not add more tasks/audits to an anxious tracker user
    if (us09) {
        if (rec.rec_type === "REC-11")
            return "US-09_active_defer_audit";
    }
    // ── US-12: Framework Gap → suppress product recs until framework in place ─
    if (us12) {
        if (rec.rec_type === "REC-10")
            return "US-12_framework_not_established";
        if (rec.rec_type === "REC-11")
            return "US-12_framework_not_established";
        if (rec.rec_type === "REC-26")
            return "US-12_framework_not_established";
    }
    // ── US-16: Illness → suspend deficit warnings ─────────────────────────────
    if (us16) {
        if (rec.rec_type === "REC-14")
            return "US-16_deficit_suspended_during_illness";
        if (rec.rec_type === "REC-19")
            return "US-16_no_sleep_restriction_during_illness";
    }
 ...
 */
export function applyConflictToneAdjustments(recommendations, states) {
    const us07 = isActive(states, "US-07");
 ...
 */
export function shouldShowCycleCount(states) {
 ...
 */
export function shouldShowDeficitWarning(states) {
    if (isActive(states, "US-07"))
        return false;
