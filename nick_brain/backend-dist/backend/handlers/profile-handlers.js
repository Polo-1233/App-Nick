 ...
 */
import { createUser, createUserProfile, updateUserProfile, upsertEnvironment, updateRecommendationStatus, } from "../db/mutations.js";
 ...
 */
export async function createUserHandler(req, res, auth) {
...
// ─── POST /profile ────────────────────────────────────────────────────────────
export async function updateProfileHandler(req, res, auth) {
    const body = await readBody(req);
    if (!body) {
...
export async function updateEnvironmentHandler(req, res, auth) {
    const body = await readBody(req);
    if (!body) {
        sendError(res, 400, "Request body must be valid JSON", "INVALID_BODY");
        return;
...
export async function recommendationActionHandler(req, res, auth) {
    const body = await readBody(req);
    if (!body) {
        sendError(res, 400, "Request body must be valid JSON", "INVALID_BODY");
        return;
    ...
    const ok = await updateRecommendationStatus(auth.client, auth.userId, body.recommendation_id, body.action);
    if (!ok) {
