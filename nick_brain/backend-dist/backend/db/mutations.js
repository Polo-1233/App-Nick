...
// ─── Users ────────────────────────────────────────────────────────────────────
export async function createUser(client, authUid, timezone = "Europe/London") {
...
// ─── User Profile ─────────────────────────────────────────────────────────────
export async function createUserProfile(client, userId, input) {
...
function sanitiseProfileInput(input) {
    const out = {};
    if (input.arp_time !== undefined)
        out["arp_time"] = input.arp_time;
    if (input.arp_committed !== undefined) {
        out["arp_committed"] = input.arp_committed;
        if (input.arp_committed)
            out["arp_committed_at"] = new Date().toISOString();
    }
    if (input.chronotype !== undefined)
        out["chronotype"] = input.chronotype;
    if (input.cycle_target !== undefined)
        out["cycle_target"] = input.cycle_target;
    if (input.caffeine_use !== undefined)
        out["caffeine_use"] = input.caffeine_use;
    if (input.tracker_in_use !== undefined)
        out["tracker_in_use"] = input.tracker_in_use;
    if (input.user_reported_anxiety !== undefined)
        out["user_reported_anxiety"] = input.user_reported_anxiety;
    if (input.user_reported_tracker_anxiety !== undefined)
        out["user_reported_tracker_anxiety"] = input.user_reported_tracker_anxiety;
    if (input.user_reported_screen_use !== undefined)
        out["user_reported_screen_use"] = input.user_reported_screen_use;
    if (input.occupation_schedule !== undefined)
        out["occupation_schedule"] = input.occupation_schedule;
    return out;
}
// ─── ARP Config ───────────────────────────────────────────────────────────────
/**
 ...
 */
export async function upsertARPConfig(client, userId, config) {
...
// ─── Sleep Logs ───────────────────────────────────────────────────────────────
export async function upsertSleepLog(client, userId, input) {
    const payload = {
        user_id: userId,
        date: input.date,
    };
    if (input.wake_time !== undefined)
        payload["wake_time"] = input.wake_time;
    if (input.actual_sleep_onset !== undefined)
        payload["actual_sleep_onset"] = input.actual_sleep_onset;
    if (input.arp_maintained !== undefined)
        payload["arp_maintained"] = input.arp_maintained;
    if (input.disruption_event_id !== undefined)
        payload["disruption_event_id"] = input.disruption_event_id;
    if (input.night_waking_2_to_4am !== undefined)
        payload["night_waking_2_to_4am"] = input.night_waking_2_to_4am;
    if (input.onset_latency_minutes !== undefined)
        payload["onset_latency_minutes"] = input.onset_latency_minutes;
    if (input.pre_sleep_routine_done !== undefined)
        payload["pre_sleep_routine_done"] = input.pre_sleep_routine_done;
    if (input.post_sleep_routine_done !== undefined)
        payload["post_sleep_routine_done"] = input.post_sleep_routine_done;
    if (input.subjective_energy_on_waking !== undefined)
        payload["subjective_energy_on_waking"] = input.subjective_energy_on_waking;
    if (input.notes !== undefined)
        payload["notes"] = input.notes;
    // cycles_completed: use provided value, or formula result, or null (never coerce to 0)
    const finalCycles = input.cycles_completed !== undefined
        ? input.cycles_completed
        : input.computed_cycles !== undefined
            ? input.computed_cycles
            : undefined;
    if (finalCycles !== undefined)
        payload["cycles_completed"] = finalCycles;
    const { data, error } = await client
        .from("sleep_logs")
...
// ─── Daily Logs ───────────────────────────────────────────────────────────────
export async function upsertDailyLog(client, userId, input) {
    const payload = {
        user_id: userId,
        date: input.date,
    };
    if (input.mrm_count !== undefined)
        payload["mrm_count"] = input.mrm_count;
    if (input.crp_taken !== undefined)
        payload["crp_taken"] = input.crp_taken;
    if (input.crp_duration_minutes !== undefined)
        payload["crp_duration_minutes"] = input.crp_duration_minutes;
    if (input.crp_start_time !== undefined)
        payload["crp_start_time"] = input.crp_start_time;
    if (input.crp_in_window !== undefined)
        payload["crp_in_window"] = input.crp_in_window;
    if (input.morning_light_achieved !== undefined)
        payload["morning_light_achieved"] = input.morning_light_achieved;
    if (input.evening_light_managed !== undefined)
        payload["evening_light_managed"] = input.evening_light_managed;
    if (input.caffeine_doses !== undefined)
        payload["caffeine_doses"] = input.caffeine_doses;
    if (input.caffeine_last_time !== undefined)
        payload["caffeine_last_time"] = input.caffeine_last_time;
    if (input.caffeine_after_cutoff !== undefined)
        payload["caffeine_after_cutoff"] = input.caffeine_after_cutoff;
    if (input.subjective_energy_midday !== undefined)
        payload["subjective_energy_midday"] = input.subjective_energy_midday;
    if (input.subjective_energy_evening !== undefined)
        payload["subjective_energy_evening"] = input.subjective_energy_evening;
    if (input.disruption_event_id !== undefined)
        payload["disruption_event_id"] = input.disruption_event_id;
    const { data, error } = await client
        .from("daily_logs")
 ...
 */
export async function upsertWeeklyBalance(client, userId, weekStart, weekEnd, accounting, nocturnalPerDay, crpPerDay) {
    const { error } = await client
 ...
 */
export async function syncUserStates(client, userId, detectedStates) {
 ...
 */
export async function persistRecommendations(client, userId, recommendations) {
    const now = new Date().toISOString();
 ...
 */
export async function updateCooldown(client, userId, recType) {
    await client
        ...
        cooldown_hours: REC_COOLDOWN_HOURS[recType] ?? 24,
    }, { onConflict: "user_id,rec_type" });
}
/**
 * Handle user action on a recommendation (actioned / dismissed).
 */
export async function updateRecommendationStatus(client, userId, recommendationId, action) {
    const now = new Date().toISOString();
    const updatePayload = action === "actioned"
    ...
    if (action === "dismissed") {
        ...
        await client
            .from("recommendation_cooldowns")
            .upsert({ user_id: userId, rec_type: recType, dismissed_count: newCount }, { onConflict: "user_id,rec_type" });
    }
    return true;
}
// ─── Events ───────────────────────────────────────────────────────────────────
export async function createEvent(client, userId, input) {
    const { data, error } = await client
        .from("event_contexts")
        .insert({
        user_id: userId,
...
export async function resolveEvent(client, userId, eventId) {
    const { error } = await client
        .from("event_contexts")
        .update({ active: false })
        .eq("id", eventId)
...
// ─── Environment ──────────────────────────────────────────────────────────────
export async function upsertEnvironment(client, userId, input) {
    const payload = { user_id: userId };
    if (input.bedroom_temperature !== undefined)
        payload["bedroom_temperature"] = input.bedroom_temperature;
    if (input.evening_light_environment !== undefined)
        payload["evening_light_environment"] = input.evening_light_environment;
    if (input.tv_in_bedroom !== undefined)
        payload["tv_in_bedroom"] = input.tv_in_bedroom;
    if (input.work_items_in_bedroom !== undefined)
        payload["work_items_in_bedroom"] = input.work_items_in_bedroom;
    if (input.blackout_provision !== undefined)
        payload["blackout_provision"] = input.blackout_provision;
    if (input.dws_device !== undefined)
        payload["dws_device"] = input.dws_device;
    const { error } = await client
