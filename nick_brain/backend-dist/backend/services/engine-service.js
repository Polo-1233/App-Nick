...
import { syncUserStates, persistRecommendations, updateCooldown, upsertARPConfig, upsertWeeklyBalance, } from "../db/mutations.js";
/**
 ...
 */
export async function runAndPersistEngine(client, userId, now) {
 ...
 */
async function persistEngineOutput(client, userId, ctx, output) {
    ...
    if (output.arp_config && (!ctx.arp_config ||
    ...
    if (output.weekly_accounting) {
        ...
        const resolvedWeekStart = computeWeekStart(ctx.profile.arp_committed_at?.slice(0, 10) ?? ctx.today, ctx.today);
        ...
        await upsertWeeklyBalance(client, userId, resolvedWeekStart, weekEnd, acct, nocturnalPerDay, crpPerDay);
    }
}
// ─── Helpers ──────────────────────────────────────────────────────────────────
/**
 ...
 */
export function computeWeekStart(arpCommittedAt, today) {
 ...
 */
export async function generateAndPersistARPConfig(client, userId, arpTime) {
