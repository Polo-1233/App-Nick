...
import type { SleepLogInput, DailyLogInput, CheckInInput } from "../types.js";
...
export async function submitSleepLogHandler(
  req: IncomingMessage,
  res: ServerResponse,
  auth: AuthContext
): Promise<void> {
  const body = await readBody<SleepLogInput>(req);
  if (!body) {
...
export async function submitDailyLogHandler(
  req: IncomingMessage,
  res: ServerResponse,
  auth: AuthContext
): Promise<void> {
  const body = await readBody<DailyLogInput>(req);
  if (!body) {
    sendError(res, 400, "Request body must be valid JSON", "INVALID_BODY");
    return;
  }
...
export async function submitCheckInHandler(
  req: IncomingMessage,
  res: ServerResponse,
  auth: AuthContext
): Promise<void> {
  const body = await readBody<CheckInInput>(req);
  if (!body) {
    sendError(res, 400, "Request body must be valid JSON", "INVALID_BODY");
    return;
  }
