...
function toChronotype(v: string | null | undefined): Chronotype {
  if (v === "AMer" || v === "PMer" || v === "In-betweener") return v;
  return "Unknown";
}

function toEventType(v: string): EventType {
  const valid: EventType[] = ["travel", "illness", "injury", "social_disruption", "shift_change", "pre_event"];
  return valid.includes(v as EventType) ? (v as EventType) : "social_disruption";
}

function toBedroomTemp(v: string | null | undefined): BedroomTemperature | null {
  if (v === "hot" || v === "comfortable" || v === "cool" || v === "variable") return v;
  return null;
}

function toEveningLight(v: string | null | undefined): EveningLightEnvironment | null {
  if (v === "bright_blue" || v === "mixed" || v === "amber_managed") return v;
  return null;
}

function toOccupationSchedule(v: string | null | undefined): OccupationSchedule | null {
  const valid: OccupationSchedule[] = ["standard", "early_starts", "flexible", "freelance", "shift_work"];
  if (v && valid.includes(v as OccupationSchedule)) return v as OccupationSchedule;
 ...
 */
export async function assembleEngineContext(
  client: AppClient,
  userId: string,
  now?: string
  ...
  const profile: UserProfile = {
    ...
    caffeine_use: (profileRow.caffeine_use as UserProfile["caffeine_use"]) ?? null,
    ...
    active_shift: (profileRow.active_shift as "day" | "night" | null) ?? null,
  ...
  // ── Map ARPConfig ─────────────────────────────────────────────────────────
  const arpConfig: ARPConfig | null = arpConfigRow
    ? {
  ...
  // ── Map SleepLogs ─────────────────────────────────────────────────────────
  const sleepLogs: SleepLog[] = rawSleepLogs.map(row => ({
    id: row.id,
    user_id: row.user_id,
    date: row.date,
  ...
  // ── Map DailyLogs ─────────────────────────────────────────────────────────
  const dailyLogs: DailyLog[] = rawDailyLogs.map(row => ({
    id: row.id,
    user_id: row.user_id,
    date: row.date,
  ...
  const weeklyBalance: WeeklyCycleBalance | null = weeklyBalanceRow
    ? {
  ...
  // ── Map EventContexts ─────────────────────────────────────────────────────
  const events: EventContext[] = rawEvents.map(row => ({
    id: row.id,
    user_id: row.user_id,
  ...
  const environment: EnvironmentContext | null = environmentRow
    ? {
  ...
  // ── Map Cooldowns ─────────────────────────────────────────────────────────
  const cooldowns: RecommendationCooldown[] = rawCooldowns.map(row => ({
    rec_type: row.rec_type as RecommendationCooldown["rec_type"],
    last_delivered_at: row.last_triggered_at,  // DB: last_triggered_at
    dismissed_count: row.dismissed_count,
  }));

  // ── App usage days ────────────────────────────────────────────────────────
  const appUsageDays = daysBetween(
    userRow.created_at.slice(0, 10),
    today
  ) + 1;
