...
export function detectPatterns(summaries: WeeklySummaryRow[]): string[] {
  if (summaries.length === 0) return [];

  const patterns: Array<{ priority: number; text: string }> = [];
  ...
  // 4. Best week
  let bestWeek: WeeklySummaryRow | null = null;
  for (const s of sorted) {
