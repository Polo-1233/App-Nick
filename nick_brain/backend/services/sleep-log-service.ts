...
export async function submitSleepLog(
  ...
  userId: string,
  input: SleepLogInput
): Promise<ServiceResult<SleepLogResult>> {
  // ── Validate ────────────────────────────────────────────────────────────
  const validation = validateSleepLogInput(input);
  if (!validation.ok) return validation as unknown as ServiceResult<SleepLogResult>;

  // ── Resolve cycles_completed ─────────────────────────────────────────────
  let computedCycles: number | null | undefined;
  ...
  let arpMaintained: boolean | undefined;
  if (input.wake_time) {
...
function validateSleepLogInput(input: SleepLogInput): ServiceResult<null> {
  ...
  if (
    input.subjective_energy_on_waking !== undefined &&
