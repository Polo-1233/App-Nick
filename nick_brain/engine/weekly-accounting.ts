 ...
 */
export function computeDayNumber(today: string, weekStart: string): number {
 ...
 */
function resolvedCycles(log: SleepLog): number | null {
  if (log.cycles_completed != null) return log.cycles_completed;
 ...
 */
function resolvedCRPCredit(log: DailyLog): number {
  if (!log.crp_taken) return 0;
  // DB trigger may have computed this
  if (log.crp_cycle_credited != null) return log.crp_cycle_credited ? 1 : 0;
  // Engine fallback
  if (log.crp_duration_minutes != null && log.crp_duration_minutes >= 20) return 1;
  return 0;
}
 ...
 */
function computeARPStable(sleepLogs: SleepLog[]): boolean {
  const wakeTimes = sleepLogs
    .map(l => l.wake_time)
    .filter((t): t is string => t != null)
    .map(timeToMin);
  if (wakeTimes.length < 2) return true; // not enough data — assume stable
 ...
 */
export function computeWeeklyAccounting(
  sleepLogs: SleepLog[],
  dailyLogs: DailyLog[],
  dayNumber: number
 ...
 */
export function consecutiveNightsBelow(
  sleepLogs: SleepLog[],  // sorted descending (most recent first)
  threshold: number
): number {
  let count = 0;
  for (const log of sleepLogs) {
    const cycles = resolvedCycles(log);
    if (cycles === null) continue;  // skip missing data
    if (cycles < threshold) {
      count++;
    } else {
      break;
 ...
 */
export function countHighLatencyNights(
  sleepLogs: SleepLog[],  // sorted descending
  threshold: number,
  lastN: number
): number {
  return sleepLogs.slice(0, lastN).filter(
    l => l.onset_latency_minutes != null && l.onset_latency_minutes > threshold
  ).length;
}
 ...
 */
export function countCaffeineAfterCutoff(
  dailyLogs: DailyLog[],  // sorted descending
  lastN: number
): number {
 ...
 */
export function meanMRMCount(dailyLogs: DailyLog[], lastN: number): number {
 ...
 */
export function countCRPTaken(dailyLogs: DailyLog[], lastN: number): number {
