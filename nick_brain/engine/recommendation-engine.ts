...
const COOLDOWN_HOURS: Partial<Record<RecommendationType, number>> = {
...
const CAP_EXEMPT: Set<RecommendationType> = new Set(["REC-01", "REC-20"]);

// Priority mapping: CRITICAL=1, HIGH=2, MEDIUM=3, LOW=4
const REC_PRIORITY: Record<RecommendationType, { priority: number; label: PriorityLevel }> = {
...
function isOnCooldown(
  ...
  if (!cd?.last_delivered_at) return false;
...
function dismissedCount(recType: RecommendationType, ctx: EngineContext): number {
  return ctx.cooldowns.find(c => c.rec_type === recType)?.dismissed_count ?? 0;
}

function buildRec(
  recType: RecommendationType,
  triggeredBy: UserStateId | null,
...
function isFrameworkEstablished(ctx: EngineContext): boolean {
...
function earliestCRPSlot(ctx: EngineContext): string {
  if (!ctx.arp_config) return "14:00"; // safe default
...
function tryREC02(
  ...
  if (!ctx.profile.arp_committed || !ctx.arp_config) return null;
  if (isOnCooldown("REC-02", ctx)) return null;
...
function tryREC03(
  ...
  if (isOnCooldown("REC-03", ctx)) return null;
  ...
  if (crpAlreadyTaken) return null;
  ...
  const deficitTrigger =
    (weekly_balance?.cycle_deficit ?? 0) >= 3 &&
    (weekly_balance?.day_number ?? 0) <= 5;

  // OR: US-06 active
  const disruption = isActive(states, "US-06");

  if (!shortNight && !deficitTrigger && !disruption) return null;
...
function tryREC04(
  ...
  if (!isActive(states, "US-12")) return null;
  const duringOnboarding = ctx.profile.onboarding_step <= 3;
  if (!duringOnboarding && isOnCooldown("REC-04", ctx)) return null;
...
function tryREC13(
  ...
  if (isOnCooldown("REC-13", ctx)) return null;

  const trigger =
    isActive(states, "US-07") ||
...
function tryREC14(
  ...
  if (isOnCooldown("REC-14", ctx)) return null;
  if (isActive(states, "US-07")) return null; // suppress metric display under anxiety

  const trigger =
    isActive(states, "US-02") || isActive(states, "US-03");
  if (!trigger) return null;
...
function tryREC15(
  ...
  if (!isActive(states, "US-07") && isOnCooldown("REC-15", ctx)) return null;

  const { sleep_logs } = ctx;
  const lastNight = sleep_logs[0];
  const latencyTrigger =
    lastNight?.onset_latency_minutes != null &&
    lastNight.onset_latency_minutes > 15;

  if (!isActive(states, "US-07") && !latencyTrigger) return null;
...
function tryREC16(
  ...
  if (isOnCooldown("REC-16", ctx)) return null;
  const lastNight = ctx.sleep_logs[0];
  if (!lastNight?.night_waking_2_to_4am) return null;
...
function tryREC17(
  ...
  if (isOnCooldown("REC-17", ctx)) return null;
  if (!isActive(states, "US-10")) return null;
...
function tryREC18(
  ...
  if (!isActive(states, "US-09")) return null;
  if (isOnCooldown("REC-18", ctx)) return null;
...
function tryREC19(
  ...
  if (ctx.app_usage_days < 14) return null;

  if (isOnCooldown("REC-19", ctx)) return null;
  ...
  if (!us03State || us03State.active_days < 14) return null;
...
function tryREC20(
  ...
  if (!isActive(states, "US-12") && ctx.profile.arp_committed) return null;
...
function tryREC21(
  ...
  if (!isActive(states, "US-05")) return null;
  if (isOnCooldown("REC-21", ctx)) return null;

  const canShiftARP =
    ctx.profile.occupation_schedule === "flexible" ||
...
function tryREC22(
  ...
  if (!isActive(states, "US-06")) return null;
  if (isActive(states, "US-03")) return null; // US-03 structural takes priority
  if (isOnCooldown("REC-22", ctx)) return null;
...
function tryREC23(
  ...
  if (!isActive(states, "US-15")) return null;
...
function tryREC24(
  ...
  if (!isActive(states, "US-16")) return null;
...
function tryREC25(
  ...
  if (!isActive(states, "US-02") && !isActive(states, "US-03")) return null;
  if (isOnCooldown("REC-25", ctx)) return null;
  const dayNumber = ctx.weekly_balance?.day_number ?? 0;
  if (dayNumber < 3) return null;

  return buildRec(
    "REC-25",
    isActive(states, "US-03") ? "US-03" : "US-02",
    { recovery_day: true }
  );
}
...
function tryREC08(
  ...
  if (isActive(states, "US-07")) return null; // suppress — reduces anxiety
  if (!ctx.arp_config) return null;
  if (isOnCooldown("REC-08", ctx)) return null;
...
function tryREC09(
  ...
  if (!isFrameworkEstablished(ctx)) return null;
  if (isOnCooldown("REC-09", ctx)) return null;
  if (!isActive(states, "US-08") && !isActive(states, "US-11")) return null;
  if (ctx.environment?.evening_light_environment !== "bright_blue") return null;
  if (ctx.daily_logs[0]?.evening_light_managed) return null;
...
function tryREC10(
  ...
  if (!isFrameworkEstablished(ctx)) return null;
  if (!isActive(states, "US-11")) return null;
  if (isOnCooldown("REC-10", ctx)) return null;

  const temp = ctx.environment?.bedroom_temperature;
  if (temp !== "hot" && temp !== "variable") return null;
...
function tryREC11(
  ...
  if (!isFrameworkEstablished(ctx)) return null;
  if (!isActive(states, "US-11")) return null;
  if (isOnCooldown("REC-11", ctx)) return null;
...
function tryREC12(
  ...
  if (!isActive(states, "US-05")) return null;
  if (isOnCooldown("REC-12", ctx)) return null;

  const canShift =
    ctx.profile.occupation_schedule === "flexible" ||
    ctx.profile.occupation_schedule === "freelance";
  if (!canShift) return null; // suppress if fixed schedule
...
export function generateCandidateRecommendations(
  ...
  const push = (r: RecommendationOutput | null) => {
  ...
  const seen = new Map<RecommendationType, RecommendationOutput>();
 ...
 */
export function applyRecommendationCap(
